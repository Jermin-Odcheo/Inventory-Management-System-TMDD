...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($isAjax) {
        // This prevents any HTML output for AJAX requests
        ob_clean();
        header('Content-Type: application/json');
    }

    // Remove DepartmentID since it's now auto-generated by the database.
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'create') {
        ...
        if ($DepartmentName === '' || $DepartmentAcronym === '') {
            $response = [
                'status'  => 'error',
                'message' => 'Both department name and acronym are required.'
            ];
            echo json_encode($response);
            exit;
        }
        ...
        try {
            // 4) Check if an active department with the same name & acronym exists
            ...
            // 5) Insert + audit inside a transaction
            $pdo->beginTransaction();
        ...
        // Check RBAC permission for modify
        if (!$canModify) {
            echo json_encode([
                'status' => 'error',
