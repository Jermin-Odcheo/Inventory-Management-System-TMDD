...
<script>
    ...
    $(document).on('click', '#confirmRestoreBtn', function () {
        if (restoreId && restoreModule) {
            ...
            if (restoreModule === 'Equipment Location') {
                restoreUrl = '../../modules/equipment_manager/restore_equipment_location.php';
            } else if (restoreModule === 'Equipment Status') {
                restoreUrl = '../../modules/equipment_manager/restore_equipment_status.php';
            } else if (restoreModule === 'Equipment Details') {
                restoreUrl = '../../modules/equipment_manager/restore_equipment_details.php';
            } else {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status && response.status.toLowerCase() === 'success') {
                        // Reload the archive table so the restored row disappears
    ...
    $(document).on('click', '#confirmDeleteBtn', function () {
        if (deleteId && deleteModule) {
            var deleteUrl = '../../modules/equipment_manager/delete_equipment.php';
            var data = { id: deleteId, permanent: 1, module: deleteModule };
            $.ajax({
                url: deleteUrl,
                method: 'POST',
                data: data,
